...
1. X86有几个特权级？
   + 四个，ring0/ring1/ring2/ring3
   + 但是一般操作系统只用到内核态（ring0）和用户态（ring3）


2. 不同特权级有什么区别？
   + 有些指令只能在特定的特权级下使用，例如特权指令
   + 指令在不同特权级下可以访问的数据范围不同
   + 指令在不同特权级下行为不同


3. 请说明CPL、DPL和RPL在中断响应、函数调用和指令执行时的作用。
   + 访问中断时：CPL要低于门的DPL，大于门后段的DPL
   + 访问段时：CPL和RPL均要低于DPL
