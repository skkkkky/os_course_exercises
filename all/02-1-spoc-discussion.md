...
-  请描述在“计算机组成原理课”上，同学们做的MIPS CPU是从按复位键开始到可以接收按键输入之间的启动过程。
   -  按复位键之后，CPU主程序首先调用flash把存在flash中的监控程序代码写入到IM指令内存中，之后直接从IM的0地址的第一条指令开始读取，通过串口和PC端的监控程序建立联系，能够成功运行监控程序之后即可以通过串口在两者之间进行通讯，对相应的IM和DM进行修改和执行相关指令，从而运行监控程序提供的各种功能。
-  x86中BIOS从磁盘读入的第一个扇区是是什么内容？为什么没有直接读入操作系统内核映像？
   -  BIOS完成自检和自启动之后，会从磁盘的引导扇区读取操作系统的加载程序，一共512字节。之后再由加载程序吧操作系统从硬盘中到内存中。
   -  因为BIOS完成启动后，文件系统没有建立，所以BIOS无法确定硬盘中存放的内容，同时，硬盘中可能存在多个操作系统，BIOS无法判断启动具体的哪一个操作系统，所以需要靠引导程序帮助。
-  比较UEFI和BIOS的区别。
   -  UEFI是统一可扩展固件接口，是一种个人电脑系统规格，用来定义操作系统与系统固件之间的软件界面，作为BIOS的替代方案。
